...
public class CreateSubPackagePlugin extends PluginAdapter {
	...
	public static final String KEY_PACKAGE_PROPERTY = "keySubPackage";
	public static final String KEY_CLASS_SUFFIX_PROPERTY = "keyClassSuffix";
	...
	public boolean validate(List<String> warnings) {
		...
		mapperXmlProperties.validate(properties.getProperty(MAPPER_PACKAGE_PROPERTY),
				properties.getProperty(MAPPER_CLASS_SUFFIX_PROPERTY));
		...
		keyProperties.validate(properties.getProperty(KEY_PACKAGE_PROPERTY),
				properties.getProperty(KEY_CLASS_SUFFIX_PROPERTY));

		return modelProperties.isEnabled() || mapperProperties.isEnabled() || mapperXmlProperties.isEnabled() || exampleProperties.isEnabled() || keyProperties.isEnabled();
	}

	@Override
	public void initialized(IntrospectedTable introspectedTable) {
		introspectedTable.setBaseRecordType(modelProperties.setTypes(introspectedTable.getBaseRecordType()));
		introspectedTable.setMyBatis3JavaMapperType(mapperProperties.setTypes(introspectedTable.getMyBatis3JavaMapperType()));
//		introspectedTable.setMyBatis3SqlProviderType(mapperXmlProperties.setTypes(introspectedTable.getMyBatis3SqlProviderType()));
		introspectedTable.setExampleType(exampleProperties.setTypes(introspectedTable.getExampleType()));
		introspectedTable.setPrimaryKeyType(keyProperties.setTypes(introspectedTable.getPrimaryKeyType()));
		introspectedTable.setMyBatis3XmlMapperPackage(mapperXmlProperties.setTypes(introspectedTable.getMyBatis3XmlMapperPackage().replaceAll(".entity", "")+"."));
	}
	...
	boolean renameElementAttribute(XmlElement element, String attributeName) {
		for (int i = 0; i < element.getAttributes().size(); i++) {
			...
			if (attributeName.equals(attribute.getName())) {
				...
				element.getAttributes().set(i, keyProperties.renameAttribute(element.getAttributes().get(i)));
				element.getAttributes().set(i, mapperXmlProperties.renameAttribute(element.getAttributes().get(i)));
			}
		}
